...
const HEAVY_REQUEST_BODY = `{ user: {"firstName":"Foo","name":"Bar","email":"foo.bar@mail.com","password":"123456789","captcha":"${'a'.repeat(
    24897
)}"}}`;
...
const minimalConnectorRequestMock: ConnectorRequest = {
    url: 'http://localhost:3000/user',
    method: 'POST',
    request: {
        headers: REQUEST_HEADERS,
...
const noMethodConnectorRequestMock = {
    ...connectorRequestMock,
    method: undefined,
};

const heavyConnectorRequestMock = {
    ...connectorRequestMock,
    request: {
        headers: REQUEST_HEADERS,
        body: HEAVY_REQUEST_BODY,
    },
};
