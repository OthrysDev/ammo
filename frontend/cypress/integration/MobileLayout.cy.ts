...
import { mobile } from '../support/utils';

onlyOn(mobile(), () => {
    describe('Responsiveness of the mobile layout', () => {
        before(() => {
            cy.visit('/');
        ...
        it('Visiting root URL and receiving a bullet - should display the responsive root layout with only the bullets visible', () => {
            ...
            cy.shouldNotBeInViewport(
                '[data-cy^=main-pannel-right-grid-bullet-]'
            );
        });
        ...
        it('Clicking on scripts button - should display scripts', () => {
            ...
            cy.shouldNotBeInViewport(
                '[data-cy^=main-pannel-left-grid-bullet-]'
            );
